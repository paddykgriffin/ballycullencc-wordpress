...
?>
<div id="<?php echo $args['id']; ?>" class="bg-light wpfc-sermon-filters<?php echo $args['classes']; ?>">
	<div class="container">
		<div class="row d-lg-none">
			<div class="col">
				<button class="btn btn-outline-primary d-block w-100 " type="button" data-toggle="collapse" data-target="#collapseFilters" aria-expanded="false" aria-controls="collapseExample">
					
				</button>
			</div>
		</div>	
		<div class="row collapse mt-3 d-lg-flex" id="collapseFilters">
			<?php foreach ( $filters as $filter ) : ?>
				...
				<?php if ( ( ! empty( $args[ $filter['taxonomy'] ] ) && 'none' !== $args['visibility'] ) || empty( $args[ $filter['taxonomy'] ] ) ) : ?>
					<div class="col-md-6 col-lg mb-2 <?php echo $filter['className']; ?>" >
						<form action="<?php echo $args['action']; ?>" method="get">
							<select class="select-css" name="<?php echo $filter['taxonomy']; ?>"
